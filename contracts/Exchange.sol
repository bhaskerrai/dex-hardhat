...
contract Exchange is ERC20 {
    ...
    function addLiquidity(uint256 amountOfToken) public payable returns(uint256) {
        ...
        uint256 minTokenAmountRequired = (msg.value * ethReserveBalance) / ethReservePriorToFunctionCall;
    ...
    function getOutputAmountFromSwap(
        ...
        uint256 outputAmountWithFee = (outputAmount * 99) / 100; //Uniswap takes a 0.03% fees from each swap. Let's say we take 1% to keep things simple. Adding fees to the contract is as simple as making a few edits to our price calculation formula:

        return outputAmountWithFee;
    }
